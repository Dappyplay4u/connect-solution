...
#
# Stream name  : tfc-retail-connect-tccivr-agent-events-datastream-ue1
# Firehose name: tfc-retail-connect-tccivr-agent-events-deliverystreams-ue1
###############################################################################
...
resource "aws_iam_role_policy" "firehose" {
  ...
  name = "${local.prefix}-${local.account}-connect-${local.lob}-firehose-policy"
...
resource "aws_kinesis_firehose_delivery_stream" "ctr" {
  ...
  extended_s3_configuration {
    ...
    cloudwatch_logging_options {
      enabled         = true
      log_group_name  = "/aws/kinesisfirehose/${local.prefix}-${local.account}-connect-${local.lob}-agent-events-deliverystreams-${local.aws_region_abbr}"
